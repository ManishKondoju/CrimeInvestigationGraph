...
from langgraph_agent import CrimeInvestigationAgent
...
def init_agent():
    try:
        return CrimeInvestigationAgent()
    except Exception as e:
        st.error(f"Error initializing CrimeInvestigationAgent: {str(e)}")
        return None
...
if current_page == 'Dashboard':
    ...
    st.markdown("Ask questions in natural language - powered by a LangGraph agent")

    # Check if the agent initialized (fails if Neo4j is unreachable, etc.)
    if agent is None:
        st.error("⚠️ AI Assistant failed to initialize")
        st.info("""
        **Setup Instructions:**
        1. Confirm Neo4j is reachable (NEO4J_URI/USER/PASSWORD in .env)
        2. Get a free Groq API key: https://console.groq.com/keys
        3. Add to `.env`: `GROQ_API_KEY = "gsk_your-key"`
        4. Restart app
        """)
        st.stop()
    ...
    if 'pending_question' in st.session_state and st.session_state.pending_question:
        ...
        with st.spinner("🔍 Analyzing..."):
            result = agent.ask_with_context(question, st.session_state.conversation_context)
    ...
    if prompt := st.chat_input("Ask about crimes, suspects, gangs, evidence..."):
        ...
        with st.chat_message("assistant"):
            with st.spinner("🔍 Investigating..."):
                result = agent.ask_with_context(prompt, st.session_state.conversation_context)
                answer = result['answer']
                cypher = result.get('cypher_queries', [])
                context_data = result.get('context', {})
